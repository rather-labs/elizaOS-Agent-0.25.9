...
describe("EVAA Supply Action", () => {
    ...
    beforeAll(async () => {
        ...
        supplyAction = new SupplyAction(walletProvider);
        mockedRuntime = {
    ...
    beforeEach(() => {
        ...
        // Mock the supply method to call the callback with success status for normal tests
        vi.spyOn(SupplyAction.prototype, 'supply').mockImplementation(async (params, runtime, callback) => {
    ...
    it("should successfully supply TON and invoke callback with success", async () => {
        // Mock the waitForPrincipalChange method
        vi.spyOn(supplyAction as any, 'waitForPrincipalChange').mockResolvedValue({
        ...
        await supplyAction.supply(params, mockedRuntime, callback);
    ...
    it("should successfully supply USDT and invoke callback with success", async () => {
        // Mock the waitForPrincipalChange method
        vi.spyOn(supplyAction as any, 'waitForPrincipalChange').mockResolvedValue({
            principal: 1000000000n,
            data: {
    ...
    it("should successfully supply USDC and invoke callback with success", async () => {
        // Mock the waitForPrincipalChange method
        vi.spyOn(supplyAction as any, 'waitForPrincipalChange').mockResolvedValue({
            principal: 1000000000n,
            data: {
    ...
    it("should handle errors during supply and invoke callback with error", async () => {
        // Create a specific mock for the error case
        vi.spyOn(SupplyAction.prototype, 'supply').mockImplementationOnce(async (params, runtime, callback) => {
            if (callback) {
                callback({
                    status: "error",
                    text: "Failed to supply: Supply failed",
                    error: true
                });
            }
